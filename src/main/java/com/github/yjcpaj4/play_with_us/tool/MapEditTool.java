...
public class MapEditTool extends GraphicLooper implements MouseListener, MouseWheelListener, MouseMotionListener, KeyListener {
    ...
    protected void draw(long delta, Graphics2D g2d) {
        ...
        for (Darkness o : mResource.getDarkness()) {
        ...
        if (mResource.hasSpawn()) {
            final int SIZE = 40;
            
            Point2D p = mResource.getSpwan();
    ...
    public void keyReleased(KeyEvent e) {
        if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_Z) {
            ...
            mResource.setSpawn(new Point2D(getMousePosition()));
        }
            ...
            if (mSelectMode == SELECT_LIGHTLESS) {
                for(Polygon o : new Polygon(mSelection.getPoints(false)).getTriangulate()) {
                    mResource.addDarkness(o.getPoints());
                }
